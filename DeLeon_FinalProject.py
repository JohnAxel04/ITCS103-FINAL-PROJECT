...
def save():
    ...
    exp = productExpiry.get()
    opt = typeOption.get()
    wrk = op.load_workbook("DeLeon_Database.xlsx")
    sheet = wrk.active
    ...
    refresh()
def refresh():
    work = op.load_workbook("DeLeon_Database.xlsx")
    sheet = work.active
...
def update():
    ...
    for i in sheet.iter_rows(min_row=2):
        if str(i[0].value) == str(id):
            ...
            i[5].value = opt
            i[6].value = total
    work.save("DeLeon_Database.xlsx")
    productEntry.delete(0,tk.END)
    productquatity.delete(0,tk.END)
    productPrice.delete(0,tk.END)
    productExpiry.delete(0,tk.END)
...
mainlabel = tk.Label(window,text="Inventory Management System",font=("Poppins",18,"bold"))
...
options = ["Piece","Box","Pack","Sachet","Bottle","Can","Jar","Bag","Carton","Tube","Tray","Bundle","Roll","Set","Gallon"]
...
style.configure("Treeview",background = "white",foreground = "black",rowheight = 25,fieldbackground = "white",font = ("Poppins",10))
...
table.tag_configure("Box", background="#87CEEB")
table.tag_configure("Pack", background="#90EE90")
table.tag_configure("Sachet", background="#FFFACD")
table.tag_configure("Bottle", background="#AFEEEE")
table.tag_configure("Can", background="#C0C0C0")
table.tag_configure("Jar", background="#FFB6C1")
table.tag_configure("Bag", background="#F5DEB3")
table.tag_configure("Carton", background="#D2B48C")
table.tag_configure("Tube", background="#DDA0DD")
table.tag_configure("Tray", background="#F0E68C")
table.tag_configure("Bundle", background="#FFA07A")
table.tag_configure("Roll", background="#FFD700")
table.tag_configure("Set", background="#E6E6FA")
table.tag_configure("Gallon", background="#B0C4DE")
...
footerLabel = tk.Label(footer,text="©JohnAxel04 | 2026",background="darkgrey")
footerLabel.pack()
window.mainloop()